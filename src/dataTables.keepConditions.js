 ...
 * @version     1.2.1b // prev 1.2.0
 * @file        dataTables.keepConditions.js
 * @author      Original: Justin Hyland, Modernized: [hadithmv]
 * @license     MIT
 * @url         https://github.com/jhyland87/DataTables-Keep-Conditions, Modernized: [https://github.com/hadithmv/DataTables-Keep-Conditions/blob/master/src/dataTables.keepConditions.js]

...

my code gives me these errors
change the code by updating the relevant part so i dont get the errors

DT-COMB.min.js:38  Uncaught TypeError: $.isArray is not a function
    at KeepConditions.conditions (DT-COMB.min.js:38:10247)
    at KeepConditions._keyMap (DT-COMB.min.js:38:2544)
    at new KeepConditions (DT-COMB.min.js:38:422)
    at HTMLDocument.<anonymous> (DT-COMB.min.js:38:10778)
    at HTMLDocument.dispatch (DT-COMB.min.js:2:37728)
    at h.handle (DT-COMB.min.js:2:35746)
    at Object.trigger (DT-COMB.min.js:2:61303)
    at HTMLTableElement.<anonymous> (DT-COMB.min.js:2:61912)
    at C.each (DT-COMB.min.js:2:2844)
    at C.fn.init.each (DT-COMB.min.js:2:1361)
	
The error occurs because $.isArray is not available in your jQuery version. In newer versions of jQuery (3.0+), $.isArray() was deprecated in favor of the native Array.isArray() method.
...
Uncaught TypeError: $.unique is not a function
    at KeepConditions.getEnabledConditions (DT-COMB.min.js:38:6550)
    at KeepConditions.attachEvents (DT-COMB.min.js:38:3477)
    at KeepConditions._init (DT-COMB.min.js:38:2383)
    at new KeepConditions (DT-COMB.min.js:38:542)
    at HTMLDocument.<anonymous> (DT-COMB.min.js:38:10814)
    at HTMLDocument.dispatch (DT-COMB.min.js:2:37728)
    at h.handle (DT-COMB.min.js:2:35746)
    at Object.trigger (DT-COMB.min.js:2:61303)
    at HTMLTableElement.<anonymous> (DT-COMB.min.js:2:61912)
...
In newer versions of jQuery (since jQuery 3.0), $.unique() has been deprecated and replaced with $.uniqueSort().
...
class KeepConditions {
  ...
  constructor(dtSettings) {
    ...
    if (
      !$.fn.DataTable.isDataTable(dtSettings) &&
     ...
     */
    if (dtSettings instanceof $.fn.dataTable.Api) this._dtApi = dtSettings;
    else this._dtApi = new $.fn.dataTable.Api(dtSettings);
  ...
  static structureHash(e_dtSettings, retrieve) {
    ...
    // If this is from a jQuery event, then the data should be handed to us
      ...
      typeof e_dtSettings.type !== "undefined" &&
    ...
    // If its just a Table selector or something, get the new API instance
      ...
      $.isPlainObject(e_dtSettings) &&
      typeof $.isPlainObject(e_dtSettings.oKeepConditions)
    )
    ...
    $.each(conditions, (i, c) => {
      if (dtSettings.oKeepConditions.conditions()[c].isset()) {
        let conHashVal = dtSettings.oKeepConditions
          .conditions()
          [c].newHashVal();
        ...
        if (typeof conHashVal !== "undefined" && conHashVal !== false)
          tableHash.push(
            dtSettings.oKeepConditions.conditions()[c].key + conHashVal
          );
      }
    });
  ...
  _init() {
    ...
    if (
      this._dtSettings.oInit.keepConditions === true ||
      typeof this._dtSettings.oInit.keepConditions === "string" ||
      Array.isArray(this._dtSettings.oInit.keepConditions) ||
      ($.isPlainObject(this._dtSettings.oInit.keepConditions) &&
        (typeof this._dtSettings.oInit.keepConditions.attachEvents ===
          "undefined" ||
  ...
  _collectEnabled() {
    ...
    $.each(this.conditions(), (sCondition, oCondition) => {
      ...
      if (!this._isEnabled(sCondition) || !oCondition.isInit()) return;
  ...
  _isEnabled(condition) {
    ...
    if (condition.length === 1) {
      ...
      if (!condition)
        throw new Error(
          `Unable to find an existing condition with the key '${condition}'`
        );
    ...
    // If were verifying by the name, and the name wasn't found...
      ...
      throw new Error(
        `Unable to find an existing condition with the name '${condition}'`
      );
    }
    ...
    return (
      ...
      (Array.isArray(options) && $.inArray(condition, options) !== -1) ||
      ...
      ($.isPlainObject(options) &&
        typeof options.conditions === "string" &&
  ...
  attachEvents() {
    ...
    $.each(conditions, (sCondition, oCondition) => {
      ...
      this._dtApi.on(
        `${oCondition.event}.${this._eventNamespace}`,
        eventParams,
  ...
  attachEvent(condition) {
    ...
    // Whatever else
      ...
      console.warn(
        `Illegal parameter type for KeepConditions.attachEvent(), should be array or string, was: ${typeof condition}`
      );
    }
  }
  ...
  processHash() {
    ...
    $.each(KeepConditions.queryString(), (table, cons) => {
      ...
      if (Array.isArray(cons) || $.isPlainObject(cons)) cons = cons[0];

      // Skip to the next hash element if this one isn't for the current table
      if (table !== this._tableId) return;
      ...
      $.each(cons.split(":"), (i, c) => {
        ...
        if ($.inArray(conName, this.getEnabledConditions()) === -1) return;
  ...
  disableCondition(condition, structureHash) {
    ...
    if (Array.isArray(condition)) {
      $.each(condition, (i, c) => {
        ...
        if (this.conditions(c) !== false) {
          this._enabledConditions.splice(
            $.inArray(c, this._enabledConditions),
            1
          );
      ...
      if (this.conditions(condition) !== false) {
        this._enabledConditions.splice(
          $.inArray(condition, this._enabledConditions),
          1
        );
  ...
  getEnabledConditions() {
    return this._enabledConditions.length > 0
      ? $.uniqueSort(this._enabledConditions)
      : false;
  }
  ...
  conditions(con) {
    ...
    var conditions = {
      ...
      search: {
        ...
        onLoad: (hashComponent) => {
          if (
            typeof hashComponent !== "undefined" &&
      ...
      length: {
        ...
        isset: () =>
          ...
          _parent._dtApi.page.len() !==
            (_parent._dtSettings.oInit.pageLength ||
              _parent._dtDefaults.iDisplayLength),
      ...
      page: {
        ...
        // Check if a value for this condition is currently set for the table (and not at default)
        isset: () =>
          _parent._dtApi.page.info() && _parent._dtApi.page.info().page !== 0,
            ...
            _parent._dtApi
              ...
              .each((value, index) => {
                ...
                if (isVis === "t") {
                  ...
                  else _parent._dtApi.column(value).visible(true);
                }
                // Parse as hidden list
                else {
                  if ($.inArray(value.toString(), columns) === -1)
                    _parent._dtApi.column(value).visible(true);
                  else _parent._dtApi.column(value).visible(false);
                }
              });
      ...
      scroller: {
        ...
        // Check if a value for this condition is currently set for the table (and not at default)
        isset: () =>
          Math.trunc(parseInt(_parent._dtSettings.oScroller.s.baseRowTop)) !==
          0,

        // Return the new value to be stored in the hash for this conditions component
        newHashVal: () => {
          let scrollPos = Math.trunc(
            parseInt(_parent._dtSettings.oScroller.s.baseRowTop)
          );
      ...
      colorder: {
        ...
        onLoad: (hashComponent) => {
          ...
          if (
            JSON.stringify(hashColOrder) !==
            JSON.stringify(_parent._dtApi.colReorder.order())
          ) {
        ...
        isset: () => {
          ...
          return (
            JSON.stringify(_parent._dtApi.colReorder.order()) !==
            JSON.stringify(_parent._dtApi.columns().indexes().toArray())
          );
        },
        ...
        newHashVal: () => {
          let sequence = _parent._dtApi.colReorder.order(),
            ...
            compileColl = () => {
              ...
              else ret = collection[0] + "-" + lastInCol(1);
      ...
      order: {
        ...
        onLoad: (hashComponent) => {
          if (typeof hashComponent !== "undefined") {
            ...
            _parent._dtApi.order([
              parseInt(hashComponent.substring(1)),
              dir[hashComponent.charAt(0)],
            ]);
        ...
        isset: () =>
          ...
          JSON.stringify(_parent._dtApi.order()) !==
            JSON.stringify($.fn.dataTable.defaults.aaSorting),

        // Return the new value to be stored in the hash for this conditions component
        newHashVal: () =>
          _parent._dtApi.order()[0][1].charAt(0) + _parent._dtApi.order()[0][0],
      },
    };
    ...
    else if (Array.isArray(con) && con.length > 0) {
      var result = {};
...
((window, document, $, undefined) => {
    ...
    language: {
      ...
      keepConditions: {
        button: {
          ...
          btnCopyBody:
            "The URL with the DataTables conditions has been " +
  ...
  $(document).on("init.dt", (e, dtSettings) => {
    ...
    if (
      dtSettings.oInit.keepConditions === undefined ||
  ...
  $.fn.dataTable.Api.register(
    ...
    function (condition, updateHash) {
      return this.iterator("table", function (dtSettings) {
        return dtSettings.oKeepConditions.enableCondition(
          condition,
          updateHash
        );
      });
  ...
  $.fn.dataTable.Api.register(
    ...
    function (condition, updateHash) {
      return this.iterator("table", function (dtSettings) {
        return dtSettings.oKeepConditions.disableCondition(
          condition,
          updateHash
        );
      });
    ...
    action: (e, dt, node, config) => {
      var dtLanguage = dt.settings()[0].oLanguage.keepConditions,
        ...
        language = {
          ...
          btnNoHashBody:
            dtLanguage.btnNoHashBody || "Thre are no conditions to be copied",
          btnCopyTitle: dtLanguage.btnCopyTitle || "URL Copied",
          btnCopyBody:
            dtLanguage.btnCopyBody ||
          ...
          btnSelectBody:
            dtLanguage.btnSelectBody ||
            "Copy be below input to easily share the URL",
        };
